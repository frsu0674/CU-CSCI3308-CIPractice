...
double coord_2d_area_triangle(const coord_2d_t* a, const coord2d_t* b, const coord_2d_t* c) {
    if(!a) {
        DEBUG(__FILE__,__LINE,__func__, "'a' must not be NULL");
        return NAN;
    }
    if(!b) {
        DEBUG(__FILE__,__LINE,__func__, "'b' must not be NULL");
        return NAN;
    }
    if(!c) {
        DEBUG(__FILE__,__LINE,__func__, "'c' must not be NULL");
        return NAN;
    }
